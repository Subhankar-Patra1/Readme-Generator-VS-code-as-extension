...
export async function exportAsPdf(
    ...
    try {
        ...
        vscode.window.showInformationMessage(
            'README exported as HTML. Open in browser and use Print → Save as PDF.',
            'Open File'
        ).then(action => {
            if (action === 'Open File') {
 ...
 */
function generatePrintableHtml(markdown: string): string {
...
export async function promptAndExport(
    ...
    const format = await vscode.window.showQuickPick(
        [
            { label: '📄 Markdown (.md)', value: 'md' as ExportFormat },
            { label: '📝 Plain Text (.txt)', value: 'txt' as ExportFormat },
            { label: '🖨️ HTML (Print to PDF)', value: 'pdf' as ExportFormat }
        ],
        {
