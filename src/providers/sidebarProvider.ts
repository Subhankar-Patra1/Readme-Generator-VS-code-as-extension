...
import { getAllTemplates, ALL_SECTIONS } from '../templates/templates';
import { hasApiKey, hasHuggingFaceApiKey, setHuggingFaceApiKey } from '../utils/secretStorage';
import { getVersions } from '../history/historyManager';

export class SidebarProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'ai-readme.sidebarView';
    ...
    public async refresh(): Promise<void> {
        ...
        // Scan workspace
        const projectInfo = await scanWorkspace();
        ...
        const detection = detectAll(projectInfo);
        const projectType = detectProjectType(projectInfo, detection);
        const hasKey = await hasHuggingFaceApiKey();
        const versions = await getVersions(projectInfo.rootPath);
