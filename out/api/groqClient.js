...
exports.generateWithGroq = generateWithGroq;
exports.testGroqConnection = testGroqConnection;
 ...
 */
async function generateWithGroq(messages, callbacks) {
    // We're reusing the HuggingFace key storage for Groq key
    const apiKey = await (0, secretStorage_1.getHuggingFaceApiKey)();
    if (!apiKey) {
            ...
            if (!response.ok) {
                ...
                if (response.status === 429) {
                    if (switchToNextModel()) {
                        ...
                        resetToFirstModel();
                        callbacks.onError(new Error('⏳ Rate limit reached on all models.\n\n' +
