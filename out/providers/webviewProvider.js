...
const readmeGenerator_1 = require("../generator/readmeGenerator");
const promptBuilder_1 = require("../generator/promptBuilder");
...
class WebviewProvider {
     ...
     */
    async createOrShow(targetFolder) {
        ...
        // Create new panel
        this._panel = vscode.window.createWebviewPanel(WebviewProvider.viewType, 'AI README Generator', column || vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        this._panel.webview.onDidReceiveMessage(message => this.handleMessage(message), undefined, this._context.subscriptions);
        // Handle disposal
    ...
    async refreshData() {
        ...
        // Scan workspace
        this._projectInfo = await (0, workspaceScanner_1.scanWorkspace)(this._targetFolder);
        ...
        this._detection = (0, languageDetector_1.detectAll)(this._projectInfo);
        this._projectType = (0, projectTypeDetector_1.detectProjectType)(this._projectInfo, this._detection);
        ...
        // Get history
        const versions = await (0, historyManager_1.getVersions)(this._projectInfo.rootPath);
        ...
        this._panel.webview.postMessage({
            ...
            data: {
                ...
                templates: (0, templates_1.getAllTemplates)().map(t => ({
                    id: t.id,
                    name: t.name,
                    description: t.description,
                    icon: t.icon
                })),
                sections: templates_1.ALL_SECTIONS,
    ...
    async handleMessage(message) {
        switch (message.type) {
            ...
            case 'refine':
                ...
                break;
            case 'saveApiKey':
                const { setHuggingFaceApiKey } = await Promise.resolve().then(() => __importStar(require('../utils/secretStorage')));
    ...
    async handleGenerate(options) {
        ...
        await (0, readmeGenerator_1.generateReadme)(this._projectInfo, this._detection, this._projectType, options, 
        // onToken
        (token) => {
        ...
        async (content) => {
            ...
            // Save to history
            if (this._projectInfo) {
                await (0, historyManager_1.saveVersion)(this._projectInfo.rootPath, content);
                const versions = await (0, historyManager_1.getVersions)(this._projectInfo.rootPath);
                this._panel?.webview.postMessage({
                    type: 'historyUpdated',
                    data: {
     ...
     */
    async handleRefine(instruction, currentContent) {
        if (!this._panel) {
            return;
        }
        ...
        const { systemPrompt, userPrompt } = (0, promptBuilder_1.buildRefinementPrompt)(instruction, currentContent);
        const messages = [
        ...
        const hasGroqKey = await (0, secretStorage_1.hasHuggingFaceApiKey)();
     ...
     */
    async handlePreviewPrompt(options) {
        if (!this._panel || !this._projectInfo || !this._detection || !this._projectType) {
            return;
        }
        const prompt = (0, readmeGenerator_1.getPromptPreview)(this._projectInfo, this._detection, this._projectType, options);
        this._panel.webview.postMessage({
            type: 'promptPreview',
            data: { prompt }
        });
    ...
    async handleSave(data) {
        ...
        const exists = await (0, diffHelper_1.readmeExists)(this._projectInfo.rootPath);
        if (exists) {
            const action = await (0, diffHelper_1.promptForExistingReadme)();
            switch (action) {
                case 'diff':
                    await (0, diffHelper_1.showDiff)(this._projectInfo.rootPath, content);
                    return;
                case 'cancel':
                    return;
                case 'overwrite':
        ...
        const saved = await (0, diffHelper_1.saveReadme)(this._projectInfo.rootPath, content);
        if (saved) {
            vscode.window.showInformationMessage('README.md saved successfully!');
            await (0, diffHelper_1.openReadme)(this._projectInfo.rootPath);
    ...
    async handleShowDiff() {
        if (!this._projectInfo || !this._generatedContent)
            return;
        const existing = await (0, diffHelper_1.getExistingReadme)(this._projectInfo.rootPath);
        await (0, diffHelper_1.showDiff)(this._projectInfo.rootPath, this._generatedContent, existing || undefined);
    }
    /**
     * Handle export
     */
    async handleExport() {
        ...
        await (0, fileExporter_1.promptAndExport)(content, this._projectInfo.rootPath);
    }
    /**
     * Handle load version
     */
    async handleLoadVersion(versionId) {
        if (!this._projectInfo || !this._panel)
            return;
        const content = await (0, historyManager_1.getVersionContent)(this._projectInfo.rootPath, versionId);
        if (content) {
            this._generatedContent = content;
            this._panel.webview.postMessage({
                type: 'generated',
     ...
     * Handle rollback
     */
    async handleRollback(versionId) {
        if (!this._projectInfo)
            return;
        const confirm = await vscode.window.showWarningMessage('Are you sure you want to restore this version? This will overwrite the current README.md.', { modal: true }, 'Restore');
        if (confirm !== 'Restore')
            return;
        const success = await (0, historyManager_1.rollbackToVersion)(this._projectInfo.rootPath, versionId);
        if (success) {
     ...
     */
    getInlineWebviewContent() {
        return getWebviewHtml();
    }
}
exports.WebviewProvider = WebviewProvider;
WebviewProvider.viewType = 'ai-readme.webview';
/**
